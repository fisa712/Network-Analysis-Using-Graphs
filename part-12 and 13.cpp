...
void BFS_Strongly_connected_componenet(int Source)//algo
{
		static int time=0;
		static int count = 0;
		...
		if(l->total==d->total)
		{
			...
			//count = count + 1;
			//SSC_Size.add_edge(count + 1, Size);
...
void Data_to_graph()
{
	ifstream myfile;
	string raw;
	//myfile.open("p12_13/CA-GrQc.txt", ios::in);///                      CA-GrQc                  //             undirected_graph_data
	myfile.open("p12_13/Sir.txt", ios::in);///
	if (myfile){
...
//	undirected_graph.Display_All_nodes();
	...
	cout<<"Do you want to print Outdegree? (y/n): ";
	cin >> ch;
	Out_degree();
	return ;
}
